...
export function MonthlyReportButton({
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed inset-0 z-[80] grid place-items-center overflow-y-auto bg-black/50 p-4 backdrop-blur-sm"
          role="dialog"
          aria-modal="true"
          aria-label="Download monthly report"
        >
          <div className="card my-8 w-full max-w-2xl overflow-hidden p-6 shadow-2xl">
            <div className="flex items-center justify-between border-b pb-4">
              <div className="flex items-center gap-3">
                <span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#c7f36b] text-black">
                  <FileSpreadsheet size={20} />
                </span>
                <div>
                  <p className="eyebrow">Financial & Operations Export</p>
                  <h2 className="display text-2xl font-bold">Monthly Report</h2>
                </div>
              </div>
              <button
                type="button"
                ...
                className="grid h-8 w-8 place-items-center rounded-full border hover:bg-[#f1efe8]"
              >
                <X size={15} />
              </button>
            </div>
            ...
            <div className="mt-5 space-y-5">
              ...
              <div className="flex items-center justify-end gap-3 border-t pt-4">
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="rounded-xl border px-4 py-2.5 text-xs font-bold hover:bg-[#f1efe8]"
                >
                  Cancel
                </button>
                <a
                  ...
                  className="flex items-center gap-2 rounded-xl bg-black px-5 py-2.5 text-xs font-bold text-white shadow-md transition hover:bg-[#2c2c28]"
                >
                  <Download size={15} />
                  <span>Download {monthNames[selectedMonth - 1]} {selectedYear} Report (CSV)</span>
                </a>
              </div>
            </div>
          </div>
